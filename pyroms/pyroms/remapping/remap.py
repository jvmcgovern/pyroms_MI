...
def remap(src_array, remap_file, src_grad1=None, src_grad2=None,
    ...
    if (ndim == 2):
        ...
        # # Following 4 lines inserted by Joe McGovern to counteract the massive negative spval in Mercator Ocean products
        # # prior to interpolation
        # src_nanidx = np.where(tmp_src_array < -100)
        # src_nonanx = np.where(tmp_src_array >= -100)
        # src_spval = np.mean(tmp_src_array[src_nonanx])
        # tmp_src_array[src_nanidx] = src_spval
        # # JVMCG mod ends here
        ...
        if iorder == 2:
            ...
            if map_method == 'conservative':
                ...
                pyroms.remapping.scrip.remap(tmp_dst_array, map_wts,
                                             dst_add, src_add, tmp_src_array, tmp_src_grad1, tmp_src_grad2)
        ...
        # reshape
        dst_array = np.reshape(tmp_dst_array, (dst_grid_dims[1], dst_grid_dims[0]))
        ...
        for k in range(nlev):
            ...
            tmp_dst_array = np.zeros(dst_grid_size)
            # # Following 4 lines inserted by Joe McGovern to counteract the massive negative spval (-32767 or -32.767)
            # # in Mercator Ocean products prior to interpolation
            # src_nanidx = np.where(tmp_src_array < -100)
            # src_nonanx = np.where(tmp_src_array >= -100)
            # src_spval = np.mean(tmp_src_array[src_nonanx])
            # tmp_src_array[src_nanidx] = src_spval
