...
if method==1; methodstr = 'quantile mapping'; end
...
if (method==0 || method==1)
    ...
    for i=1:ntclimc %Loop over climatological time divisions (e.g. month-of-year)
        if (seasonal==1)
            ...
            selt = find(dist1<=tol_dtclim); %Choose all data within tolerance tol_dtclim of climatological time centre tclimc(i)
        else
            selt = 1:ntp;
        end
...
for i=1:ntclimc
    ...
    elseif (method==2) %Repeated Hindcast Correction
        ...
        if seasonal==1; selthref1 = find(dist1<=tol_dtclimp); else selthref1 = 1:nthref; end
    ...
    for i=1:ntclimc
        ...
        for j=1:ndivdec %Loop over decadal divisions to apply linear detrending and adjust residual interannual variance.
            if (j==ndivdec)
                ...
                seltp2 = seltp(yearp(seltp)>=yearpmin+(j-1)*dyear_iavar & yearp(seltp)<yearpmin+j*dyear_iavar);
            end
...
if (use_subdelta==1 && ~isempty(Xhf) && ~isempty(tdhf))
    ...
    if all(diff(tdh)==1)
        delta_timescale = 'daily'; nqsub = 24;
    elseif all(diff(monthh)==1|diff(monthh)==-11)
        delta_timescale = 'monthly'; nqsub = 28;
    else
    ...
    if (correct_substd==1)
        ...
        for k=1:ndivt_subq %Loop over months (if opt_subq.bymonth=1)
            for i=1:ns
                rec = find(~isnan(Xhsub(:,i)) & ~isnan(dXhfstdsub(:,i)));
                ...
                if (opt_subq.doplot==1)
                    ...
                    elseif (strcmp(opt_subq.model,'ksmooth')==1)
                        ...
                        %yphat1 = Xplin(xp1,opt_subq.xp,1)*squeeze(yphatm(k,:,i))';
                    end

                    figure(ifig0); subplot(3,4,k);plot(Xhsub(rec,i),dXhfstdsub(rec,i),'k.',xp1,yphat1,'r-','LineWidth',2)
                end
            end
    ...
    elseif (correct_subquantiles==1)
        ...
        for k=1:ndivt_subq %Loop over months (if opt_subq.bymonth=1)
            if opt_subq.bymonth==1; rec = find(monthhsub==k); else rec = 1:ntsub; end
            nrec = length(rec);
    ...
    if strcmp(delta_timescale,'daily')==1; difftdhf = 1/24; end
    ...
    for j=1:length(yearpminv)
        ...
        if (match_subdelta_deltamean==0) %If not matching by day, identify the hindcast indices to repeat, and calculate correcion factors (if not correcting quantiles).
            ...
            if (allow_block_lag==1)
                ...
                ihsub1 = find(abs(dtdhsub-dtdp1)<=tol_dtclimp); %Starting index within repeated hindcast series (deltascale)
            else
                ihsub1 = 1;
            end
            ...
            if (correct_subquantiles==0) %Identify indices for finescale data and repeated finescale data for this block
                ...
                if (strcmp(delta_timescale,'monthly')==1 && length(dXhf1(:,1))==(ntf1-1))
                    dXhf1 = [dXhf1; dXhf1(end,:)]; %#ok<AGROW> %Repeat last day if necessary due to varying no. leap years per block.
