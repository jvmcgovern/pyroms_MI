...
def get_section(lonsec, latsec, var, lon, lat, rmask, h, zeta, theta_s, theta_b, hc, N, vtransform):
    ...
    for i in np.arange(0, Npts):
        ...
        if np.size(isgood) > 4:
            zetasec[i] = griddata((londata[isgood], latdata[isgood]), zetadata[isgood], (lonsec[i], latsec[i]),
                                  method='cubic')
            HSEC[i] = griddata((londata[isgood], latdata[isgood]), hdata[isgood], (lonsec[i], latsec[i]),
                               method='cubic')

            for k in np.arange(0, N):
                vdata = var[k, jmin:jmax, imin:imax].ravel()
                VAR[k, i] = griddata((londata[isgood], latdata[isgood]), vdata[isgood], (lonsec[i], latsec[i]),
                                     method='cubic')
