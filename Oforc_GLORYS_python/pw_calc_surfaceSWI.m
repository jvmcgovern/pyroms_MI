...
lat = lat(:)'; lon = lon(:)'; ns = length(lat);
tday = tday(:); nt = length(tday);
if nargin<3; opt = []; end
if isfield(opt,'use_tday_lag')==1; use_tday_lag = opt.use_tday_lag; else use_tday_lag = 0; end
if isfield(opt,'year')==1; year = opt.year; else year = []; end
if isfield(opt,'f_model')==1; f_model = opt.f_model; else f_model = 1; end
if isfield(opt,'decl_model')==1; decl_model = opt.decl_model; else decl_model = 1; end
if isfield(opt,'use_eqtime')==1; use_eqtime = opt.use_eqtime; else use_eqtime = 1; end
if isfield(opt,'Q_model')==1; Q_model = opt.Q_model; else Q_model = 0; end
if isfield(opt,'cloud_model')==1; cloud_model = opt.cloud_model; else cloud_model = 0; end
if isfield(opt,'calc_hav')==1; calc_hav = opt.calc_hav; else calc_hav = 0; end
if isfield(opt,'calc_dav')==1; calc_dav = opt.calc_dav; else calc_dav = 0; end
if isfield(opt,'calc_Qs')==1; calc_Qs = opt.calc_Qs; else calc_Qs = 0; end
if isfield(opt,'JD')==1; JD = opt.JD; else JD = []; end
if isfield(opt,'e0')==1; e0 = opt.e0; else e0 = zeros(nt,ns); end %Vapour pressure (hPa=mbar)
if isfield(opt,'C')==1; C = opt.C; else C = zeros(nt,ns); end %Cloud cover (oktas, 0-1)
if isfield(opt,'ndays_year')==1; ndays_year = opt.ndays_year; else ndays_year = 365.2422; end
if isfield(opt,'ROMS_model')==1; ROMS_model = opt.ROMS_model; else ROMS_model = 0; end
if (ROMS_model==1)
    f_model = 0; decl_model = 0.1; use_eqtime = 0; Q_model = 0.1; cloud_model = 0.1;
    ndays_year = 365.2425;
end
if nargout>1; out = struct('f_model',f_model,'decl_model',decl_model,'Q_model',Q_model,'cloud_model',cloud_model,'ndays_year',ndays_year); end
...
if (Q_model==0) %Use SMS formula (Rosati and Miyakoda, 1988) as summarized in Byun and Pinardi (2007) Table 1
    tau = 0.7;
    A_a = 0.09;
    ...
    rec = find(SE(:)>0); %This is needed to deal with potential zeros of cos_theta, which will produce NaN in QDir etc.
    ...
    Q0 = QDir + QDiff;
    if (calc_hav==1) %Calculate hourly averages centred on the prediction hours
        ...
        for i=1:ns
            ...
            rec = find(SEm(:)>0); %This is needed to deal with potential zeros of cos_theta, which will produce NaN in QDir etc.
    ...
    if (calc_dav==1)
        ...
        for i=1:ns
            cos_thetam = (sin(decl)*sin(latc(i)))*ones(1,nstep) + ...
                (cos(latc(i))*cos(decl))*cos(2*pi*(dT/2:dT:1-dT/2)); %This is ~=0, so we do not need the subselection above.
...
elseif (Q_model==0.1)
    cos_theta = max(0, sin(decl*ones(1,ns)).*sin(ones(nt,1)*latc) + cos(ones(nt,1)*latc).*cos(decl*ones(1,ns)).*cos(h)); % = 'zenith' in the ROMS code
...
if (calc_Qs==1)
    if (cloud_model==0) %Reed (1977) model for cloud transmission.
        alph = 180/pi*asin(sin(decl*ones(1,ns)).*sin(ones(nt,1)*latc) + cos(ones(nt,1)*latc).*cos(decl*ones(1,ns))); %Solar altitude at noon, in degrees
