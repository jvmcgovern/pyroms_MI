...
if __name__ == '__main__':
    ...
    # cfsr_dir = '/shared/emason/NCEP-CFSR/%s/%s/' %(cfsr_version, domain)
    # cfsr_dir = '/local/tmp/3/emason/CFSR/%s/%s/' %(cfsr_version, domain)
    cfsr_dir = '/net/acoma/local/tmp/3/emason/CFSR/%s/%s/' % (cfsr_version, domain)
    ...
    for cfsr_key in list(cfsr_files.keys()):
        ...
        if cfsr_key in 'shflux':
            cfsrgrd = CfsrGrid(''.join((cfsr_dir, cfsr_files['shflux_SW_down'][0])), 'CFSR')

        elif cfsr_key in 'dQdSST':  # used for dQdSST
            # Using uspd/sustr here cos has highest resolution (0.3 deg.)
            cfsrgrd = CfsrGrid(''.join((cfsr_dir, cfsr_files['sustr'][0])), 'CFSR')
        ...
        if 1:
            ...
            for dt in time_array:
                ...
                # Wind speed (wspd, uwnd, vwnd) and stress (sustr, svstr) (bulk only)
                    ...
                    for supp_var in supp_vars:
                        if supp_var.to_be_downscaled:
                            supp_var.data = horizInterp(supp_var.tri_all,
                                                        supp_var.datain.ravel())(cfsr_wspd.points_all)
