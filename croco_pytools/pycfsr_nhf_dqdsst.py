...
def create_nc(cfsrgrd):
    ...
    for key, value in zip(list(cfsr_vars.keys()), list(cfsr_vars.values())):
...
if __name__ == '__main__':
    ...
    needs_interp = np.zeros(len(list(cfsr_files.keys())))
    ...
    # are all the same
    ni = 0
    for key, value in zip(list(cfsr_files.keys()), list(cfsr_files.values())):
