...
from py_roms2roms import vertInterp, horizInterp, bry_flux_corr, debug0, debug1, debug2, \
    ROMS, RomsGrid, WestGrid, EastGrid, NorthGrid, SouthGrid, RomsData
...
class MercatorData(RomsData):
    ...
    def _set_variable_type(self):
        self.varname = None
        for varname, vartype in zip(list(self.var_dic.keys()), list(self.var_dic.values())):
            ...
            with netcdf.Dataset(self.romsfile[ind]) as nc:
                if '3D' in self.dimtype:
                    self.datain[:] = np.ma.masked_array(
                        nc.variables[self.varname][0, :, self.j0:self.j1, self.i0:self.i1].astype(np.float64)).data
                else:
                    self.datain[:] = np.ma.masked_array(
                        nc.variables[self.varname][0, self.j0:self.j1, self.i0:self.i1].astype(np.float64)).data
        except Exception:
            with netcdf.Dataset(self.romsfile[0]) as nc:
                if '3D' in self.dimtype:
                    self.datain[:] = np.ma.masked_array(
                        ...
                        ind, :, self.j0:self.j1, self.i0:self.i1].astype(np.float64)).data
                else:
                    self.datain[:] = np.ma.masked_array(
                        nc.variables[self.varname][ind, self.j0:self.j1, self.i0:self.i1].astype(np.float64)).data
        return self
...
if __name__ == '__main__':
    ...
    if 'CELTIC_grd_v5.nc' in roms_grd:
        sigma_params = dict(theta_s=7, theta_b=0, hc=20, N=20)
        bry_filename = 'croco_bry_PHY_CELTIC.nc'
        # bry_filename = 'croco_bry_BIO_CELTIC.nc'
        obc_dict = dict(south=1, east=1, north=1, west=1)  # 1=open, 0=closed
        start_date, end_date, day_zero = '20050101', '20050131', '20050101'

    elif 'grd_croco.nc' in roms_grd:
        sigma_params = dict(theta_s=6, theta_b=0, hc=120, N=32)
        bry_filename = 'bry_croco.nc'
        obc_dict = dict(south=1, east=1, north=1, west=1)  # 1=open, 0=closed
        start_date, end_date, day_zero = '20120109', '20130421', '20120109'

    elif 'grd_lions1500.nc' in roms_grd:
        sigma_params = dict(theta_s=7, theta_b=2, hc=300, N=50)
        bry_filename = 'bry_lions1500.nc'
        obc_dict = dict(south=1, east=1, north=0, west=1)  # 1=open, 0=closed
        start_date, end_date, day_zero = '20120109', '20130421', '20120109'
    ...
    if 'ORCA' in mercator_dir:
        ...
        mercator_ssh_files = sorted(glob.glob(mercator_dir +
                                              'CMEMS_v5r1_IBI_PHY_MY_PdE_01dav_200501??_200501??_R*_RE01.nc'))
        mercator_temp_files = sorted(glob.glob(mercator_dir +
                                               'CMEMS_v5r1_IBI_PHY_MY_PdE_01dav_200501??_200501??_R*_RE01.nc'))
        mercator_salt_files = sorted(glob.glob(mercator_dir +
                                               'CMEMS_v5r1_IBI_PHY_MY_PdE_01dav_200501??_200501??_R*_RE01.nc'))
        mercator_u_files = sorted(glob.glob(mercator_dir +
                                            'CMEMS_v5r1_IBI_PHY_MY_PdE_01dav_200501??_200501??_R*_RE01.nc'))
        mercator_v_files = sorted(glob.glob(mercator_dir +
                                            'CMEMS_v5r1_IBI_PHY_MY_PdE_01dav_200501??_200501??_R*_RE01.nc'))

        # mercator_po4_files = sorted(glob.glob(mercator_dir +
        #                                       'CMEMS_v5r1_IBI_BIO_MY_PdE_01dav_200501??_200501??_R*_RE01.nc'))
        # mercator_si_files = sorted(glob.glob(mercator_dir +
        #                                      'CMEMS_v5r1_IBI_BIO_MY_PdE_01dav_200501??_200501??_R*_RE01.nc'))
        # mercator_nh4_files = sorted(glob.glob(mercator_dir +
        #                                       'CMEMS_v5r1_IBI_BIO_MY_PdE_01dav_200501??_200501??_R*_RE01.nc'))
        # mercator_fe_files = sorted(glob.glob(mercator_dir +
        #                                      'CMEMS_v5r1_IBI_BIO_MY_PdE_01dav_200501??_200501??_R*_RE01.nc'))
        # mercator_no3_files = sorted(glob.glob(mercator_dir +
        #                                       'CMEMS_v5r1_IBI_BIO_MY_PdE_01dav_200501??_200501??_R*_RE01.nc'))
        # mercator_o2_files = sorted(glob.glob(mercator_dir +
        #                                      'CMEMS_v5r1_IBI_BIO_MY_PdE_01dav_200501??_200501??_R*_RE01.nc'))
        # mercator_dissic_files = sorted(glob.glob(mercator_dir +
        #                                          'CMEMS_v5r1_IBI_BIO_MY_PdE_01dav_200501??_200501??_R*_RE01.nc'))

        balldist = 20000.  # meters

    elif 'local/tmp/3/' in mercator_dir:
        mercator_ssh_files = sorted(glob.glob(mercator_dir + 'GLOB_MERC_PHY_001_024_20??-??-??.nc'))
        mercator_temp_files = mercator_ssh_files
        mercator_salt_files = mercator_ssh_files
        mercator_u_files = mercator_ssh_files
        mercator_v_files = mercator_ssh_files
        balldist = 20000.  # meters

    # Add further elifs below according to your needs:
    ...
    timeformat = "%Y%m%d"
    fmissing = 0
    for i, f in enumerate(mercator_ssh_files):
        m = re.search(regex, f)
        dtnow = plt.date2num(datetime.strptime(m.string[m.regs[0][0]:m.regs[0][1]], timeformat))
        if i:
            dtnow = plt.date2num(datetime.strptime(m.string[m.regs[0][0]:m.regs[0][1]], timeformat))
            if (dtnow - dtstart) != 1:
                # print m.groups()[0], dtnow - dtstart
                print(mercator_ssh_files[i - 1])
                dtstart = dtnow - 1
            dtstart += 1
        else:
            dtstart = plt.date2num(datetime.strptime(m.string[m.regs[0][0]:m.regs[0][1]], timeformat))
    ...
    for boundary, is_open in zip(list(obc_dict.keys()), list(obc_dict.values())):
    ...
    #                      U=mercator_u_files,
    #                      V=mercator_v_files)
    mercator_vars = {"SSH": mercator_ssh_files,
                     "TEMP": mercator_temp_files,
                     "SALT": mercator_salt_files,
                     "U": mercator_u_files,
                     "V": mercator_v_files}
    # mercator_vars = dict(PO4=mercator_po4_files,
    #                      Si=mercator_si_files,
    #                      NH4=mercator_nh4_files,
    #                      FER=mercator_fe_files,
    #                      NO3=mercator_no3_files,
    #                      O2=mercator_o2_files,
    #                      DIC=mercator_dissic_files)

for mercator_var, mercator_files in zip(list(mercator_vars.keys()), list(mercator_vars.values())):
                ...
                for dt in mercator_dates:
                    ...
                    with netcdf.Dataset(romsbry.romsfile, 'a') as nc:
                        ...
                        if mercator.vartype in 'U':
                            ...
                            #     mercator.dataout += k2c  # Kelvin to Celsius
                            nc.variables['temp_%s' % boundary][tind] = mercator.dataout

                        elif mercator.vartype in 'SALT':

                            nc.variables['temp_%s' % boundary][tind] = mercator.dataout

                        elif mercator.vartype in 'DIC':
...
# with netcdf.Dataset(romsbry.romsfile, 'a') as nc:
with netcdf.Dataset(bry_filename, 'a') as nc:
