...
from matplotlib.path import Path as nx
import time
    ...
    def read_nc_at_index(self, varname, ind):
        ...
        try:
            with netcdf.Dataset(self.romsfile) as nc:
                ...
                return eval(''.join(("nc.variables[varname]", ind)))
        except Exception:
            try:
                with netcdf.Dataset(self.romsfile[0]) as nc:
                    # return eval(''.join(("nc.variables[varname]", indices)))
...
class RomsData(ROMS):
    ...
    def create_bry_nc(self, grdobj, obc_dict, cycle, fillval, madeby):
        ...
        for boundary, flag in zip(list(obc_dict.keys()), list(obc_dict.values())):

            if flag:

                varlabel = '%sern' % boundary

                for key, value in zip(list(prog_vars.keys()), list(prog_vars.values())):
...
if __name__ == '__main__':
    ...
    chd_bry_surface_areas = []
    for open_boundary, flag in zip(list(obc_dict.keys()), list(obc_dict.values())):
                            ...
                            for ocean_time in nc.variables['ocean_time'][dataind:]:
                                ...
                                with netcdf.Dataset(romsbry.romsfile, 'a') as ncbry:
                                    ...
                                    if 'zeta' in roms_var:
                                        ...
                                        ncbry.variables['zeta_%s' % open_boundary][
                                            tind] = czeta * cgrd_at_bry.maskr().squeeze()
                                        ocean_time /= 86400.
